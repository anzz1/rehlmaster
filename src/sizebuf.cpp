...
bool CSizeBuf::WriteString(const char* s, bool nullTerminate) {
	if (!s) s = "";
	return Write(s, strlen(s) + (nullTerminate ? 1 : 0));
}
...
void CSizeBuf::Cut(unsigned int cutpos, unsigned int cutlen) {
	if (cutpos + cutlen > m_CurSize) {
		ALERT(at_error, "CSizeBuf::Cut: Invalid cut request; cutpos=%u, cutlen=%u, curSize=%u\n", cutpos, cutlen, m_CurSize);
		return;
	}
